...
namespace MoreMountains.CorgiEngine
{
	...
	public class LevelManager : MMSingleton<LevelManager>, MMEventListener<CorgiEngineEvent>
	{
		...
		{
			// storage
			LevelCameraController = FindAnyObjectByType<CameraController>();
			...
			switch (CheckpointAttributionAxis)
			{
				case CheckpointsAxis.x:
					...
					{
						Checkpoints = FindObjectsByType<CheckPoint> ().OrderBy (o => o.transform.position.x).ToList ();
					}
					else
					{
						Checkpoints = FindObjectsByType<CheckPoint> ().OrderByDescending (o => o.transform.position.x).ToList ();
					}
					break;
				case CheckpointsAxis.y:
					if (CheckpointAttributionDirection == CheckpointDirections.Ascending)
					{
						Checkpoints = FindObjectsByType<CheckPoint> ().OrderBy (o => o.transform.position.y).ToList ();
					}
					else
					{
						Checkpoints = FindObjectsByType<CheckPoint> ().OrderByDescending (o => o.transform.position.y).ToList ();
					}
					break;
				case CheckpointsAxis.z:
					if (CheckpointAttributionDirection == CheckpointDirections.Ascending)
					{
						Checkpoints = FindObjectsByType<CheckPoint> ().OrderBy (o => o.transform.position.z).ToList ();
					}
					else
					{
						Checkpoints = FindObjectsByType<CheckPoint> ().OrderByDescending (o => o.transform.position.z).ToList ();
					}
					break;
				case CheckpointsAxis.CheckpointOrder:
					Checkpoints = FindObjectsByType<CheckPoint>().OrderBy(o => o.CheckPointOrder).ToList();
					break;
			}
		...
		protected virtual void CheckpointAssignment()
		{
			...
			IEnumerable<Respawnable> listeners = FindObjectsByType<MonoBehaviour>(FindObjectsInactive.Include).OfType<Respawnable>();
