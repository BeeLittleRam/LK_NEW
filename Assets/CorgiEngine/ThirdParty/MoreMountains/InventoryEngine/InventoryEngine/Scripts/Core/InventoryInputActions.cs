	...
	public class InventoryInputActions : MonoBehaviour
	{
		...
		public class InventoryInputActionsBindings
		{
			...
			public int SlotIndex = 0;
				...
				public Key InputKey = Key.Digit0;
				/// an alt key that will also trigger the action
				public Key AltInputKey = Key.None;
			#else
				/// the key that should trigger the action
		...
		protected virtual void DetectInput()
		{
			foreach (InventoryInputActionsBindings binding in InputBindings)
			{
				...
				#if ENABLE_INPUT_SYSTEM && !ENABLE_LEGACY_INPUT_MANAGER
					bool mainKeyPressed = binding.InputKey != Key.None && Keyboard.current[binding.InputKey].wasPressedThisFrame;
					bool altKeyPressed = binding.AltInputKey != Key.None && Keyboard.current[binding.AltInputKey].wasPressedThisFrame;
				if (mainKeyPressed || altKeyPressed)
				#else
