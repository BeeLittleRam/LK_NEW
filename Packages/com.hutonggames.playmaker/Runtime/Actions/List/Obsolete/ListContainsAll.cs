...
{
    [Obsolete("Use CheckListContainsAll instead.")]
    [Serializable]
    [PublicAPI]
    [ActionCategory(Category.List)]
    [ActionDescription("Check if a list contains all of the given items. Stores result in a bool variable.")]
    [HelpURL("https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.list-1.contains")]
    public class ListContainsAll : BaseAction
    {
        ...
        public override void Execute() => 
            Result.Value = Items.All(item => List.ListVariable.Contains(item.GetValue()));
        
        public override string GetSummary() => "{List} contains all {Items} -> {Result}";
    }
}