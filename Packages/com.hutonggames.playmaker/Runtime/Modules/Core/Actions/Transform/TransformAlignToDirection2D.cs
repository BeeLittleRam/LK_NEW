...
namespace HutongGames.PlayMaker.Actions
{
	...
	public sealed class TransformAlignToDirection2D : BaseAction
	{
		[OwnerDefaultValue]
		...
		[SerializeField]
		private AxisDirection2DVar _alignAxis;
		...
		public override void Execute()
		{
			...
			// For 2D rotation, we only care about the Z component of rotation
			// Calculate the angle from the direction vector
			var targetAngle = Mathf.Atan2(_direction.Value.y, _direction.Value.x) * Mathf.Rad2Deg;

			// Get the current axis direction to determine offset
			var fromDirection = _alignAxis.Value.GetDirection(transform);
			var fromAngle = Mathf.Atan2(fromDirection.y, fromDirection.x) * Mathf.Rad2Deg;

			// Calculate the angle difference and apply it as a Z-axis rotation
			var angleDifference = targetAngle - fromAngle;

			// Create rotation only around Z-axis
			var currentEuler = transform.rotation.eulerAngles;
			var newRotation = Quaternion.Euler(currentEuler.x, currentEuler.y, currentEuler.z + angleDifference);
			var smoothTime = SmoothTime;
			var maxSpeed = MaxSpeed;

			transform.rotation = smoothTime <= 0f && maxSpeed <= 0f
				? newRotation
				: SmoothLookAtHelper.Update(transform.rotation, newRotation, smoothTime, maxSpeed);
		}
		...
		public AxisDirection2D AlignAxis => _alignAxis.Value;
