...
namespace HutongGames.PlayMaker.Actions.SceneManagement
{
    ...
    [ActionDescription(
        "Preloads a Scene asynchronously by its name without activating it. " +
    ...
    public sealed class SceneManagerPreloadSceneByName : BaseAction
    {
        ...
        public override bool CanExecute() => CheckParameters(_sceneName);
        ...
        {
            _asyncOperation = SceneManager.LoadSceneAsync(_sceneName.Value, _parameters);
            if (_asyncOperation == null)
            {
                Debug.LogError($"SceneManagerPreloadAndActivateByIndex: Failed to start async load for {_sceneName.Value}.");
                Finish();
                yield break;
            }
        ...
        public override string GetSummary() =>
            "Preload scene {_sceneName} and wait for {_activateFlag}";
    }
}
