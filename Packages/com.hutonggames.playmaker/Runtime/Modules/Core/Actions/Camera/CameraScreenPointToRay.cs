...
namespace HutongGames.PlayMaker.Actions
{
	...
	public sealed class CameraScreenPointToRay : BaseAction
	{
		[Tooltip("The Camera.")]
		...
		public override bool CanExecute() => CheckParameters(_camera, _screenPoint, _ray);

		public override void Execute() => _ray.Value = _camera.Value.ScreenPointToRay(_screenPoint.Value);

		public override string GetSummary() => _camera.Value == Camera.main
				? "Convert screen point {_screenPoint} to ray -> {_ray}"
				: "Convert {_camera} screen point {_screenPoint} to ray -> {_ray}";
	}
}
