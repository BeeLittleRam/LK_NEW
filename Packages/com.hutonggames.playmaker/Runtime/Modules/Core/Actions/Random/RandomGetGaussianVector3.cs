...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [ActionDescription("Get a random Vector3 from a Gaussian (normal) distribution centered on Center.")]
    [HelpURL("https://en.wikipedia.org/wiki/Box%E2%80%93Muller_transform")]
    public class RandomGetGaussianVector3 : BaseAction
    {
        ...
        [WriteOnly, DefaultName("RandomPosition")]
        [Tooltip("Store the random value in a Vector3 variable.")]
        ...
        public override void Execute()
        {
            ...
            StoreResult.Value = new Vector3(
                Center.Value.x + RandomGetGaussian.SampleStandardNormal() * sd,
                Center.Value.y + RandomGetGaussian.SampleStandardNormal() * sd,
                Center.Value.z + RandomGetGaussian.SampleStandardNormal() * sd);
        }

        public override string GetSummary()
        {
            return "Get Gaussian Vector3 center:{Center} sd:{StandardDeviation} -> {StoreResult}";
        }
    }
}
