...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [ActionDescription("Find a valid random 2D position using a generator block and validator blocks.")]
    public class FindValidRandomPosition2D : BaseAction
    {
        [SerializeReference]
        [ShowActionBlockTitle]
        [DisplayName("Random Position")]
        [Tooltip("Generates candidate positions to test.")]
        public SpawnPositionGeneratorBlock2D Generator;

        [OptionalField]
        [SerializeReference]
        [DisplayName("Modifiers")]
        [Tooltip("Modifiers that can adjust the candidate position before validation.")]
        public List<SpawnPositionModifierBlock2D> Modifiers;

        [OptionalField]
        [SerializeReference]
        [DisplayName("Validators")]
        [Tooltip("Validators that must all pass for the candidate position to be accepted.")]
        public List<SpawnPositionValidatorBlock2D> Validators;
        ...
        public Vector2Ref StorePosition;
        ...
        [NonSerialized] public Vector2 CandidatePosition;
        [NonSerialized] public float CandidateRotation;
        [NonSerialized] public int AttemptIndex;

        public override bool CanExecute() =>
            Generator is { IsValid: true }
        ...
        public override void Execute()
        {
            ...
            for (var i = 0; i < maxAttempts; i++)
            {
                ...
                {
                    StoreRotation.Value = Quaternion.Euler(0f, 0f, CandidateRotation);
                }
                if (PositionFound.IsAssigned)
                {
