...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [ActionDescription("Picks up a Rigidbody2D by smoothly aligning it to a target Transform, then optionally parenting it there. " +
                       "Stores the previous kinematic and interpolation state, then applies a held-state setup.")]
    public sealed class Rigidbody2DPickUp : BaseAction
    {
        ...
        [Tooltip("The Rigidbody2D to pick up.")]
        [SerializeField]
        ...
        [Tooltip("Match the target Z rotation while picking up.")]
        [SerializeField, DefaultValue(true)]
        private BoolVar _setRotation;

        [Tooltip("Parent the Rigidbody2D transform to the target Transform after pickup completes.")]
        [SerializeField, DefaultValue(true)]
        ...
        [SerializeField, WriteOnly]
        private RigidbodyInterpolation2DRef _previousInterpolation;
        ...
        public override bool CanStart() =>
            CheckParameters(_rigidbody2D, _target, _setPosition, _setRotation, _parentAfterPickup, _zeroVelocity, _smoothDuration, _easing);

        public override bool CanExecute() =>
            CheckParameters(_rigidbody2D, _setPosition, _setRotation, _parentAfterPickup, _zeroVelocity, _smoothDuration, _easing);
        ...
        public override string GetSummary() =>
            "Pick up {_rigidbody2D} to {_target} {_pickedUpEvent}";

        private void CachePhysicsState(Rigidbody2D rb)
        {
            if (_initialized)
            {
                return;
            ...
            {
                _wasKinematic.Value = rb.GetIsKinematicShim();
            }
            ...
            rb.SetIsKinematicShim(true);
            rb.interpolation = RigidbodyInterpolation2D.None;
        }

        private void ApplyImmediatePickup(Rigidbody2D rb, Transform target, bool setPosition, bool setRotation)
        {
            if (setPosition)
            {
        ...
        private void ApplySmoothPickup(Rigidbody2D rb, Transform target, bool setPosition, bool setRotation, float t)
        {
            if (setPosition)
            {
                rb.MovePosition(Vector2.Lerp(_startPosition, (Vector2)target.position, t));
            }

            if (setRotation)
            {
                rb.MoveRotation(Mathf.LerpAngle(_startRotation, target.eulerAngles.z, t));
            }
        ...
        private void CompletePickup(Rigidbody2D rb, Transform target)
        {
            if (_completed)
            {
                return;
