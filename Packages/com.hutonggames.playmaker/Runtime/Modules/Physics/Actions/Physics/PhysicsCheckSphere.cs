...
namespace HutongGames.PlayMaker.Actions
{
	...
	[ActionDescription("Returns true if there are any colliders overlapping the sphere defined by positio" +
		"n and radius in world coordinates.")]
	[HelpURL("https://docs.unity3d.com/ScriptReference/Physics.CheckSphere.html")]
	public sealed class PhysicsCheckSphere : BaseAction
	{
		...
		[Tooltip("A Layer mask that is used to selectively ignore colliders when casting a capsule.")]
		...
		{
			return CheckParameters(_position, _radius, _layerMask);
		}
		
		public override void Execute()
		{
			var hasOverlaps = Physics.CheckSphere(_position.Value, _radius.Value, _layerMask.Value, _hitTriggers);
		...
		public override string GetSummary()
		{
			...
			{
				return "Check sphere {_position} {_radius} {_layerMask} -> {_result}";
			}
			
			var summary = _hasOverlaps.IsNone && _noOverlaps.IsSet
				? "If sphere {_position} {_radius} {_layerMask} has no overlaps {_noOverlaps}"
				: "If sphere {_position} {_radius} {_layerMask} has overlaps {_hasOverlaps}" +
