...
{
    [UsedImplicitly]
    [CustomActionEditor(typeof(DataTableGetRandomRecord))]
    public sealed class DataTableGetRandomRecordEditor : BaseDataTableWithOverrideEditor<DataTableGetRandomRecord>
    {
        private readonly List<DataSchemaUtility.SchemaField> _numericFields = new();

        protected override void BuildAfterContentGUI()
        {
            AddField(nameof(DataTableGetRandomRecord.Record));
            AddField(nameof(DataTableGetRandomRecord.OnRowNotFound));
            AddField(nameof(DataTableGetRandomRecord.Index));
            AddField(nameof(DataTableGetRandomRecord.Key));
            AddField(nameof(DataTableGetRandomRecord.Found));
            AddField(nameof(DataTableGetRandomRecord.NotFoundEvent));
        }
        ...
        protected override void BuildTableUI(DataDefinition definition)
        {
            ...
            dropdown.RegisterValueChangedCallback(_ =>
            {
                ...
                var guidProp = TargetProperty.FindPropertyRelative(nameof(DataTableGetRandomRecord.WeightFieldGuid));
                if (guidProp == null)
                    return;
        ...
        {
            if (index <= 0)
                return "Optional. Leave empty so every row has the same chance to be selected. Select a numeric field to make rows with larger values more likely to be selected.";

            var fieldTooltip = _numericFields[index - 1].Tooltip;
            return string.IsNullOrEmpty(fieldTooltip)
                ? "Rows with larger values are more likely to be selected."
                : fieldTooltip + "\n\nRows with larger values are more likely to be selected.";
        }
