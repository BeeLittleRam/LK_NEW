...
{
    [UsedImplicitly]
    [CustomActionEditor(typeof(DataRecordSetValue))]
    public sealed class DataRecordSetValueEditor : CustomActionEditor
    {
        private readonly List<DataSchemaUtility.SchemaField> _schemaFields = new();

        private DataRecordSetValue Action => Target as DataRecordSetValue;
        ...
        {
            AddField(nameof(DataRecordSetValue.Record));

            _definitionFieldElement = TargetProperty.FindPropertyRelative(nameof(DataRecordSetValue.DataDefinition)) != null
                ? AddField(nameof(DataRecordSetValue.DataDefinition))
                : null;

            ContentRoot = new VisualElement();
            Add(ContentRoot);

            AddField(nameof(DataRecordSetValue.Succeeded));
            ...
            var recordProp = TargetProperty.FindPropertyRelative(nameof(DataRecordSetValue.Record));
            if (recordProp != null)
            {
            ...
            var defProp = TargetProperty.FindPropertyRelative(nameof(DataRecordSetValue.DataDefinition));
            if (defProp != null)
            {
        ...
        private void BuildRecordUI(DataDefinition def)
        {
            ...
            var setValueProp = TargetProperty.FindPropertyRelative(nameof(DataRecordSetValue.SetValue));
        ...
        {
            var typeFromCell = GetTypeFromSelectedRecordCell(fieldGuid);
        ...
        private Type GetTypeFromSelectedRecordCell(SerializableGuid fieldGuid)
        {
            var record = Action?.Record?.Value;
            var cells = record?.Data?.Cells;
            if (cells == null)
                return null;
